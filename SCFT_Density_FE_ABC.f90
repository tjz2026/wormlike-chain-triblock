...
# endif /* SLAB */
       implicit none
       integer :: i,j,s,K,s1,s2,blk_indx
       ...
       REAL(DP),allocatable :: c_C(:)
       REAL(DP),allocatable :: c_chain(:)
       REAL(DP),allocatable :: denz(:)
       REAL(DP) :: sum_iA,sum_iB,sum_half,sum_end_B
       REAL(DP) :: sum_i(3),sum_joint(3)
       REAL(DP) :: nor_coeff
...
!# endif /* Debug */
   ...
   do K=0,LOCAL_SIZE-1
     do blk_indx=1,3
          sum_i(blk_indx)=0.0
         do i=0,max_M_Bar-1
            do s=0+Nstart_blk(blk_indx),Ns_blk(blk_indx)+Nstart_blk(blk_indx)
              c_chain(s)=q_f(i,K,s)*q_star(i,K,s) 
            enddo
          RHO_blk(i,K,blk_indx)=simposon_1D_NR(0,NA,ds,c_A)
          sum_i(blk_indx)=sum_i(blk_indx)+RHO_blk(i,K,blk_indx)
         enddo
      enddo   
        denz(K)=sum(sum_i(:))
        R_blk(K,1)=sum_i(1)
...
# endif /* Dim2 */
     ...
     do K=0,LOCAL_SIZE-1
       do blk_indx=1,3
       R_blk(k,blk_indx)=M_v*R_blk(k,blk_indx)/totden
       enddo
    enddo 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     do K=0,LOCAL_SIZE-1
         ...
         sum_end_C=0.0
      do i=0,M_Bar_A-1
          sum_end_A=sum_end_A+q_f(i,K,NA)*q_b(i,K,NA)
      enddo

      do i=0,M_Bar_B-1
          sum_end_B=sum_end_B+q_f(i,K,NA+NB)*q_b(i,K,NA+NB)
      enddo

      do i=0,M_Bar_C-1
          sum_end_C=sum_end_C+q_f(i,K,Nmax)*q_b(i,K,Nmax)
      enddo

        R_AB(K)=M_v*sum_end_A/totden
        R_BC(K)=M_v*sum_end_B/totden
        R_end(K)=M_v*sum_end_C/totden
      enddo
