# if defined (SPH)
module split_operator_MDE_single_core
implicit none
...
contains
        ...
        double precision :: temp,k_z_factor
    ...
    do i=1,Nphi
      do j=1,Ntheta+1
        do k=0,LOCAL_SIZEK-1
         ...
         temp=unit_vec(j,i)%z*kk*dz_inv*k_z_factor
        temp=-0.5d0*ds*temp
...
!for A
 allocate(exp_WA(0:SIDEz-1),stat=istat2)
!for B
 allocate(exp_WB(0:SIDEz-1),stat=istat2)
 
 allocate(q_temp1(0:SIDEz-1,1:Ntheta+1,1:Nphi),stat=istat2)
 ...
 stop
    ...
    do i=1,Nphi
     ...
     enddo   
      ...
      c_nor(0)= simposon_sph_spa1d_mpi(Ntheta,Nphi,SIDEz,dtheta,dphi, &
                                       dz,q_temp1)
      ...
      q_f(:,:,:,0)=q_f(:,:,:,0)*c_nor_inv
       ...
       call sph_onestep_MDE_single_core(s,exp_WA,exp_L_A,1)
       enddo ! s=1,Nmax

       do s=NA+1,Nmax
       call sph_onestep_MDE_single_core(s,exp_WB,exp_L_B,1)
       enddo ! s=1,Nmax
    ...
    do i=1,Nphi
     ...
     enddo   
      ...
      deallocate(q_temp1)
       ...
       subroutine sph_onestep_MDE_single_core(s,exp_W,exp_L_temp,orientation)
       USE nrtype,only :DP,PI
       USE global_para
       USE mpi
       USE mmpi
       ...
       real(DP) :: q_temp2(0:SIDEz-1)
       integer,intent(in) ::  s
       real(DP),intent(in) :: exp_W(0:SIDEz-1)
     ...
     allocate(q_temp_out(0:SIDEz-1,1:Ntheta+1,1:Nphi))
        ...
        ! step 1 and 2, q multiply exp_wA and then a pair of fft 
           do i=1,Nphi
             do j=1,Ntheta+1
                ...
                do k=0,SIDEz-1
                 in_1d(k)=dcmplx(q_temp_out(k,j,i)*exp_W(k),0.0d0)
                enddo

                call dfftw_execute_dft(plan_1d_f, in_1d, out_1d)

                 do k=0,SIDEz-1
                out_1d(k)=dcmplx(exp_u_dot_k_r(k,j,i), & 
                          orientation*exp_u_dot_k_i(k,j,i))* & 
                         out_1d(k) 
                 enddo
                

                call dfftw_execute_dft(plan_1d_b, out_1d,in_1d)

                do k=0,SIDEz-1
                 q_temp_out(k,j,i)=real(in_1d(k))*M_grid_inv
                enddo 
            ...
            do i=1,Nphi
             do j=1,Ntheta+1
                ...
                do k=0,SIDEz-1
                 in_1d(k)=dcmplx(q_temp_out(k,j,i),0.0d0)
                enddo
...
end module split_operator_MDE_single_core
# endif /* SPH */
